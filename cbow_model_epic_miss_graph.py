...
if __name__ == '__main__':
    ...
    data_col = kfold_miss_node_split(unique_codes, params.node_drop_fold, codes_ind, texts_ind, labels, g)
    ...
    def learn_one_fold(train,test,val):
        ...
        if params.finetune_method == 'partial':
            task_train_learner = MLPGraphForwardLearner('t_task', train, graph_pred_model, task_loss, Adam, params.task_batch_size)
        else:
            task_train_learner = GraphForwardLearner('t_task', train, graph_pred_model, task_loss, Adam, params.task_batch_size)
    ...
    for drop, g_l in comb:
        ...
        for i in range(params.node_drop_fold):
            train, test, valid = data_col[i]

            val_res, test_res = learn_one_fold(train,test,valid)
