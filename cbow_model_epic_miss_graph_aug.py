...
from graph_text_model import CCAModel, GraphLabelingModel, GraphModel, ProjTaskModel, TextModel
...
if __name__ == '__main__':
    ...
    parser.add_argument("--node_drop_ratio", type=int, default=0.3)
    parser.add_argument("--node_drop_fold", type=int, default=5)
    ...
    codes_arr = np.array(codes_ind)

    unique_codes = np.unique(codes_arr)

    data_col = kfold_miss_node_split_unseen(unique_codes, params.node_drop_fold, codes_ind, texts_ind, labels, num_nodes, g)

    def learn_one_fold(data):
        train, task_train, test, val, code_labels = data
        print('train size: {}, test size: {}, val size: {}'.format(len(train),len(test),len(val)))
        graph_model = GraphLabelingModel(params, code_labels)
        ...
        manager.train([train_learner, val_learner], trainer, LossEvaluator('loss_eval'), eval_metric, device=params.device, num_epochs=params.pre_num_epochs, eval_every=10, save_epoch=10)
        ...
        graph_pred_model = ProjTaskModel(graph_model, V, params).to(params.device)

        if params.finetune_method == 'partial':
            task_train_learner = MLPGraphForwardLearner('t_task', task_train, graph_pred_model, task_loss, Adam, params.task_batch_size)
        else:
            task_train_learner = GraphForwardLearner('t_task', task_train, graph_pred_model, task_loss, Adam, params.task_batch_size)
        task_train_learner.setup_optimizer([{'lr':params.finetune_lr, 'weight_decay':params.l2}])

        task_test_learner = GraphForwardLearner('t_task', test, graph_pred_model, None, None, params.task_batch_size)
        task_val_learner = GraphForwardLearner('t_task', val, graph_pred_model, None, None, params.task_batch_size)
        
        manager = Manager(params.task_model_name)
        manager.train([task_train_learner, task_val_learner], task_trainer, task_evlter, task_eval_metric, device=params.device, num_epochs=params.num_epochs, eval_every=5, save_epoch=5)
    ...
    if params.psearch:
        ...
        g_layers = [params.num_gcn_layers]
    comb = itertools.product(dropout_list, g_layers)

    best_res = []
    best_config = []
    for drop, g_l in comb:
        params.dropout = drop
        params.num_gcn_layers = g_l
        val_res_col = []
        test_res_col = []
        for i in range(params.node_drop_fold):

            val_res, test_res = learn_one_fold(data_col[i])
            val_res_col.append(val_res)
            test_res_col.append(test_res)

        val_res = dict_res_summary(val_res_col)
        test_res = dict_res_summary(test_res_col)
        print(val_res)
        print(test_res)
